...
2. Then navigated to **User Menu -> Settings -> All Settings  -> Integration -> Communication Systems**

   <img src="./images/in3-3.png" alt="Pic 3-3" width=25% height=25%>

   ![Pic 3-4](./images/in3-4.png)

3. Created a **Communication System - "teched2025_aem_tenant"** and maintained the hostname and the credentials of the **SAP Integration Suite, advanced event mesh** to which the events must be dispatched. You can use the REST endpoint of SAP AEM broker obtained from the SAP AEM. Activate the communication system.

   ![Pic 3-5](./images/in3-5.png)
   
4. Next, in the **Communication Configuration** page, **Send Events to External Systems** has been configured.\
   a. Open the **Communication Configuration**,  **Send Events to External Systems**, and select **Copy**.\
   b. In the copied Communication Configuration, selected the **Communication System - "teched2025_aem_tenant"** that we created in step 3.\
   c. In the same Communication Configuration, under the **HTTP Data section**, maintained the **API path** that specify the **Topic** of the **SAP Integration Suite, advanced event mesh**\
   d. Activated the Communication Configuration.\
   c. The **host_name of the Communication System + API Path** is where the outbound events are dispatched. The event payload is dispatched to the configured endpoint as an HTTP POST call.
...
7. We have now configured the event processing, and the events raised by the application services are dispatched to the configured endpoint.

8. You can monitor the outbound events, if in case of any errors by navigating to **Admin Settings -> Integration -> Standard Events Monitoring**

...
7. If you are curious to see the entries that correspond to other parcipants and were discarded, go back to the Overview -> Monitoring Message Processing tile. Click on the '>' button located in the top left side of the screen to open a detailed panel.
...
8. You can see messages to the same IFlow, but classified with the 'Terminated' custom status. Additionally you can look at the 'attachments' section and verify that that the `SalesOrder` attribute carries a different identiifer from yours. 

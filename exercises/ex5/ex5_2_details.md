...
Head back to your Integration Suite tenants.
...
3. You may remember that we had assigned the **Application_ID** header only when a matching condition was met. Hence, you will see the message entries filtered only for your individual execution runs. This step was done to ensure that you avoid 'external noise'. You can verify that the **'Custom Status'** will always point to **'Successful: Customer ID matched'**.
...
7. (Optional step) If you are curious to see the entries that correspond to other participants and were discarded, go back to the **Overview -> Monitoring Message Processing** tile. Click on the expand **'>'** button located in the top left side of the screen to open a detailed panel.

   <img src="../ex5/images/image21.png" width=100% height=100%>

8. (Optional step) Look for the **'Custom Status'** drop-down and select **'Terminated: Customer ID mismatch'** from the value help.

   <img src="../ex5/images/image22.png" width=100% height=100%>

9. (Optional step) You can see messages to the same Integration Flow, but classified with the **'Terminated: Customer ID mismatch'** custom status. Additionally, you can look at the 'attachments' section and verify that the `SalesOrder` attribute carries a different identifier from yours. 
